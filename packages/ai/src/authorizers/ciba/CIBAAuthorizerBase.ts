...
export class CIBAAuthorizerBase<ToolExecuteArgs extends any[]> {
  ...
  constructor(
    ...
    const isInterruptMode =
      params.onAuthorizationRequest === undefined ||
      params.onAuthorizationRequest === "interrupt";

    this.authResponseStore = isInterruptMode
      ? new SubStore<CIBAAuthorizationRequest>(params.store, {
          getTTL: (authResponse) =>
            authResponse.expiresIn ? authResponse.expiresIn * 1000 : undefined,
        })
      : undefined;
    this.credentialsStore = isInterruptMode
      ? new SubStore<TokenSet>(params.store, {
          getTTL: (credentials) =>
        ...
        try {
          ...
          if (!credentials) {
            if (interruptMode) {
              ...
              authResponse = await this.start(authorizeParams);
              credentials = await this.getCredentialsPolling(authResponse);
            }
